...
This section brings together the theoretical framework of two-body dynamics through eight comprehensive worked problems covering binary star astrometry, symmetric mass ratios, numerical solution of Kepler's equation, orbital transfers, conserved vectors, and continuous thrust perturbation theory.
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[scale=0.9, >=Stealth]
		...
		\draw[->, thick] (0,-2.5) -- (0,5.5) node[above] {$V_{\text{eff}}(r)$};
		
		% L=0 curve (pure -k/r)
		\draw[dashed, red!80!black, thick, domain=0.7:7.2, samples=100] 
			plot (\x, {-2.5/\x});
		\node[red!80!black, right] at (5.5,-0.6) {$L=0$ (pure attraction)};
		
		% L > 0 curve (effective potential well)
		\draw[blue!80!black, very thick, domain=0.65:7.2, samples=150] 
			plot (\x, {-3.5/\x + 2.2/(\x*\x)});
		
		% Energy levels
		\draw[dashed, gray] (0, 1.2) -- (6.8, 1.2) node[right] {$E > 0$ (Hyperbolic)};
		\draw[dashed, green!60!black, thick] (0, -0.6) -- (6.0, -0.6) node[right] {$E < 0$ (Bound Elliptic)};
		\draw[dotted, purple, thick] (0, -1.39) -- (1.25, -1.39) node[midway, below] {$E_{\rm min}$ (Circular)};
		
		% Minimum point
		\fill[blue!80!black] (1.25, -1.39) circle (2.5pt);
		\node[below=3pt] at (1.25, 0) {$r_{\rm circ}$};
		\draw[dotted] (1.25, 0) -- (1.25, -1.39);
		
		% Effective potential box
		\node at (4.5, 4.2) {$\boxed{V_{\rm eff}(r) = -\frac{GM\mu}{r} + \frac{L^2}{2\mu r^2}}$};
	\end{tikzpicture}
	\caption{Effective potential well for $L > 0$, defining circular orbit equilibrium and turning points for bound Keplerian motion.}
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[scale=1.05, >=Stealth]
		...
		% Labels
		\node[green!60!black, rotate=60] at (-1.1, 0.65) {\textbf{Initial Orbit}};
		\node[blue!75!black, rotate=70] at (-2.5, 1.45) {\textbf{Final GEO Orbit}};
		\node[red!75!black] at (2.2, 0.8) {\textbf{Transfer Ellipse}};
	\end{tikzpicture}
...
\textbf{Problem Statement}: A satellite in circular orbit experiences a continuous thrust $\vec A = A\hat v$ parallel to velocity. Derive the rates of change $\dot a$ and $\dot e$.

\textbf{Solution}:
The specific energy rate is $\dot\epsilon = \vec A \cdot \vec v = Av$. Differentiating $\epsilon = -\mu_E/(2a)$:
\begin{equation}
	\boxed{\frac{da}{dt} = \frac{2a^2v}{\mu_E}A = 2A\sqrt{\frac{a^3}{\mu_E}}\sqrt{\frac{1+2e\cos v_f+e^2}{1-e^2}}}.
\end{equation}
Differentiating $e^2 = 1 + 2\epsilon h^2/\mu_E^2$ with $\dot{\vec h} = (A/v)\vec h$:
\begin{equation}
	\boxed{\frac{de}{dt} = \frac{A}{v}\frac{v^2h^2+2\epsilon h}{\mu_E^2 e}}.
\end{equation}
Because the derivatives depend on the instantaneous orbital phase $v_f(t)$, the system cannot be solved in closed elementary form and requires orbit-averaging or numerical integration.
