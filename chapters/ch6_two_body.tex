...
\subsection{Central force problem}
	...
	\begin{equation}
		\begin{pmatrix} \hatn\\ \hat y\\ \hat L \end{pmatrix}_{\text{Orbital Plane}} =
		\begin{pmatrix} 1&0&0\\ 0&\cos\iota&\sin\iota\\ 0&-\sin\iota&\cos\iota \end{pmatrix} \begin{pmatrix} \cos\Omega&\sin\Omega&0\\ -\sin\Omega&\cos\Omega&0\\ 0&0&1 \end{pmatrix} \begin{pmatrix} \hat\alpha\\ \hat\delta\\ \hat R \end{pmatrix}_{\text{Sky plane}}.
	...
	\]
	so $\vecA$ is conserved for the unperturbed problem — it is a \emph{fourth}, non-obvious constant of the Kepler problem (beyond $E,\vecL$), reflecting the extra "hidden" $SO(4)$ symmetry of the $1/r$ potential. Angular momentum together with Runge-Lenz vector forms the generator of $SO(4)$ group.
	...
	\begin{equation}
		\boxed{\dot{\hat{A}}=\sin f\,\rhat+\cos f\,\phihat .}\label{eq:qhat_def}
	\end{equation}
	Differentiating \eqref{eq:Ahat_f} gives
	\begin{align}
		\dot{\hat A}&=-\dot f\sin f\,\rhat+\cos f\,\dot{\rhat}-\dot f\cos f\,\phihat-\sin f\,\dot{\phihat},
	\end{align}
	with
	\begin{equation}
		\dot{\rhat}=\dot\phi\,\phihat,	\qquad	\dot{\phihat}=-\dot\phi\,\rhat.
	\end{equation}
	Thus
	\begin{align}
		\dot{\hat A}
