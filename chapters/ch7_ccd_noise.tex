...
\begin{enumerate}
	...
	\item \textbf{Dark Current Shot Noise:} Thermal vibrations in the silicon lattice
	spontaneously generate electron-hole pairs, independently of any light, at the same rate
	$R_{\rm dark}$ in every pixel --- again a uniform, per-pixel quantity. These accumulate over
	$n_{\rm pix}$ pixels and duration $t$:
	\begin{equation}
...
\end{enumerate}
	...
	statistical fluctuation of some quantity of charge carriers generated in the pixel. Dark current
	noise already introduced one thermal effect --- lattice vibrations thermally exciting electrons
	into the conduction band, where they add to the charge sitting in the pixel well and are counted
	along with the signal. Readout noise is thermal in a different sense and at a different stage: even
	once the charge in the well is fixed and no more carriers are being generated, that charge still has
	to be converted to a voltage and read out, and the electronics performing that conversion --- the
	output field-effect transistor (FET) and the analog-to-digital converter (ADC) --- have their own
	thermal (Johnson--Nyquist) and flicker ($1/f$) noise, which perturbs the voltage reading itself. So
	where dark current adds spurious \emph{charge} to the well before conversion, read noise adds
	spurious \emph{uncertainty to the conversion process}, after the charge is already fixed.
