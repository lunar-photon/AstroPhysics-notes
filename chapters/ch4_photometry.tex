...
Interstellar dust grains preferentially scatter shorter-wavelength blue photons (Rayleigh-like scattering $\sigma_{\rm dust} \propto \lambda^{-\gamma}$ with $\gamma \sim 1$). This dims and reddens the transmitted starlight. The observed distance modulus is modified by the total visual extinction $A_V$:
\begin{equation}
...
Differentiating $B_\lambda(T)$ with respect to $\lambda$ and setting $dB_\lambda/d\lambda = 0$ yields the condition:
\begin{equation}
	\frac{x e^x}{e^x - 1} = 5, \quad \text{where } x \equiv \frac{hc}{\lambda_{\max} k_B T} \approx 4.965114.
\end{equation}
...
Integrating Planck's specific intensity over all wavelengths and across the outward hemisphere ($\int_0^{2\pi}d\phi\int_0^{\pi/2}\cos\theta\sin\theta\,d\theta = \pi$) gives the total emergent surface flux:
\begin{equation}
	F_{\rm surface} = \pi \int_0^\infty B_\lambda(T)\,d\lambda = \sigma_{\rm SB} T^4,
	\label{eq:surface_flux_sb}
\end{equation}
where the \emph{Stefan-Boltzmann constant} is:
\begin{equation}
