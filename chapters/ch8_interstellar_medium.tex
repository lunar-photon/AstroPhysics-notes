...
\begin{definition}[Extinction]
	\emph{Extinction} is the total attenuation of radiant energy propagating through an astrophysical medium, defined as the sum of true absorption (conversion of radiant energy into thermal or internal excitation energy) and scattering (redirection of photons out of the line of sight):
	\begin{equation}
...
Absorption occurs either as sharp resonance-line absorption by atoms, ions, and molecules, or as continuous broadband absorption by larger dust grains. Scattering, however, is fundamentally categorized by the conservation of photon energy during the interaction.

\subsection{Physical Classification of Light Scattering Regimes}
\label{sec:scattering_classification}

In astrophysics and fundamental physics, scattering processes divide into two broad categories based on whether photon kinetic energy is conserved:

\begin{enumerate}[leftmargin=1.5em, itemsep=0.4em]
	\item \textbf{Elastic Scattering ($\Delta E = 0$):} The kinetic energy and quantum frequency of the photon are conserved in the center-of-mass frame ($\nu_{\rm scattered} = \nu_{\rm incident}$). The scattered radiation retains its exact wavelength and intrinsic color.
	\begin{itemize}[leftmargin=1.2em, itemsep=0.2em]
		\item \textbf{Rayleigh Scattering ($a \ll \lambda$):} Occurs when light interacts with particles (atoms, small molecules, or sub-wavelength grain kernels) much smaller than the radiation wavelength. The incident electric field induces an oscillating electric dipole $p = \alpha E_0$ with polarizability $\alpha \propto a^3$. By Larmor's radiative formula ($P_{\rm rad} \propto \ddot p^{\,2} \propto \omega^4$), the re-radiated power scales as $\omega^4 a^6 E_0^2 \propto a^6/\lambda^4$. Dividing by incident intensity yields the cross-section
		\begin{keyresult}
			\begin{equation}
			...
			\end{equation}
		\end{keyresult}
		Blue photons are scattered far more strongly than red photons, explaining the blue color of Earth's daytime sky, red sunsets, and blue reflection nebulae around young stellar clusters (e.g., the Pleiades).
		\item \textbf{Mie Scattering ($a \sim \lambda$):} Arises when electromagnetic waves interact with spherical or spheroidal dielectric and conducting particles (such as interstellar dust grains, water droplets, and aerosols) whose physical radii are comparable to the wavelength. The exact solution requires expanding internal and scattered electromagnetic fields in vector spherical harmonics (the Mie series). Across the optical and near-infrared, realistic interstellar grain size distributions yield an effective cross-section scaling as $\sigma_{\rm ext} \propto a^3\lambda^{-1}$. When particle sizes become substantially larger than the wavelength ($a \gg \lambda$), scattering approaches the geometric limit ($\sigma \approx 2\pi a^2$, including diffraction), where all visible wavelengths are scattered equally, making terrestrial clouds, thick water fog, and large-grain circumstellar envelopes appear white or neutral gray.
		\item \textbf{Tyndall Scattering:} Closely related to the Mie regime, this describes elastic scattering by small macroscopic particles or colloids suspended in a transparent carrier gas or liquid. Familiar examples include visible shafts of sunlight penetrating dusty rooms or smoky forests, as well as the diffuse illumination of dusty interstellar clouds by nearby starlight.
	\end{itemize}
	
	\item \textbf{Inelastic Scattering ($\Delta E \neq 0$):} Photons exchange energy with the internal quantum or collective dynamical states of the scattering medium, emerging with shifted wavelengths and frequencies ($\nu_{\rm scattered} \neq \nu_{\rm incident}$).
	\begin{itemize}[leftmargin=1.2em, itemsep=0.2em]
		\item \textbf{Raman Scattering:} Photons interact with atomic or molecular bonds, exciting or de-exciting vibrational or rotational quantum states. The scattered photon emerges at a lower frequency (Stokes scattering, $\nu < \nu_0$, photon loses energy) or higher frequency (anti-Stokes scattering, $\nu > \nu_0$, photon gains energy from an already excited molecule). Widely utilized in laboratory Raman spectroscopy for chemical fingerprinting, it also appears in astrophysical environments such as symbiotic stars and planetary nebulae, where intense ultraviolet resonance photons (e.g., $\mathrm{O\,VI}\,\lambda\lambda 1032, 1038$) Raman-scatter off neutral atomic hydrogen ($\mathrm{H\,I}$) into broad optical emission bands at $\lambda 6825\,\text{\AA}$ and $\lambda 7082\,\text{\AA}$.
		\item \textbf{Brillouin Scattering:} Occurs when light scatters inelastically off propagating acoustic phonons (coherent density fluctuations or hypersonic sound waves) in dense fluids, solids, or high-density astrophysical plasmas, imparting a Doppler-shifted frequency modulation $\Delta\nu = \pm 2\nu_0 (v_s/c)\sin(\theta/2)$, where $v_s$ is the local sound speed.
	\end{itemize}
\end{enumerate}

\paragraph{Free-electron scattering in high-energy astrophysics.} In addition to bound atoms and dust particulates, radiation interacts directly with free electrons in ionized astrophysical plasmas:
\begin{itemize}[leftmargin=1.2em, itemsep=0.2em]
	\item \textbf{Thomson Scattering ($h\nu \ll m_e c^2$):} The low-energy, classical elastic limit of photon-electron scattering. The cross-section is the universal Thomson cross-section:
	\begin{equation}
		\sigma_{\rm T} = \frac{8\pi}{3}\left(\frac{e^2}{4\pi\varepsilon_0 m_e c^2}\right)^2 = \frac{8\pi}{3}r_e^2 \approx 6.6524 \times 10^{-29}\,\mathrm{m^2},
		\label{eq:thomson_cross_section}
	\end{equation}
	which is strictly independent of photon wavelength $\lambda$. Thomson scattering provides the dominant continuous opacity in stellar interiors, hot massive stellar envelopes, accretion disks, and the Cosmic Microwave Background (CMB) last-scattering surface.
	\item \textbf{Compton and Inverse Compton Scattering ($h\nu \gtrsim m_e c^2$ or relativistic electrons):} The relativistic inelastic regime. In standard Compton scattering, a high-energy $\gamma$-ray photon transfers momentum to a stationary electron, increasing its wavelength by $\Delta\lambda = \lambda_{\rm C}(1-\cos\theta)$, where $\lambda_{\rm C} = h/(m_e c) \approx 2.426 \times 10^{-12}\,\mathrm{m}$. In \emph{Inverse Compton Scattering}, ultra-relativistic electrons ($\gamma_e \gg 1$) collide with low-energy ambient photons (radio, IR, or CMB), up-scattering them by a factor $\sim \gamma_e^2$ into X-ray and $\gamma$-ray energies (the primary emission engine of pulsar wind nebulae, supernova remnants, and active galactic nuclei jets).
\end{itemize}
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}l l l l l@{}}
		\toprule
		\textbf{Scattering Mechanism} & \textbf{Interacting Medium} & \textbf{Wavelength Scaling} & \textbf{Energy / Color Shift} & \textbf{Astrophysical Manifestation} \\
		\midrule
		\textbf{Rayleigh} & Atoms, small molecules ($a \ll \lambda$) & $\sigma \propto a^6\lambda^{-4}$ & Elastic ($\Delta E = 0$) & Blue sky, Pleiades reflection nebulae \\
		\textbf{Mie (dust)} & Dust grains ($a \sim \lambda$) & $\sigma \propto a^3\lambda^{-1}$ & Elastic ($\Delta E = 0$) & Interstellar reddening \& extinction \\
		\textbf{Geometric} & Large particles ($a \gg \lambda$) & $\sigma \propto a^2$ (achromatic) & Elastic ($\Delta E = 0$) & White clouds, circumstellar dust shells \\
		\textbf{Tyndall} & Colloids, dust suspensions & Mixed Mie/Rayleigh & Elastic ($\Delta E = 0$) & Interstellar dust lanes, nebula beams \\
		\textbf{Raman} & Molecular/atomic bonds & Specific resonances & Inelastic ($\Delta E \neq 0$) & Symbiotic stars, Raman O\,VI emission \\
		\textbf{Brillouin} & Acoustic phonons / sound waves & Doppler-shifted & Inelastic ($\Delta E \neq 0$) & Dense planetary/stellar atmospheres \\
		\textbf{Thomson} & Free electrons ($h\nu \ll m_e c^2$) & $\sigma_{\rm T} = \text{const}$ & Elastic ($\Delta E = 0$) & O-star envelopes, accretion disks, CMB \\
		\textbf{Inverse Compton} & Relativistic electrons & Up-scattered ($\nu' \sim \gamma_e^2\nu$) & Inelastic ($\Delta E > 0$) & AGN jets, pulsar wind nebulae ($\gamma$-rays) \\
		\bottomrule
	\end{tabular}%
	}
	\caption{Comprehensive taxonomy of light scattering mechanisms in physics and astrophysics.}
	\label{tab:scattering_taxomony}
	\label{tab:scattering_regimes}
\end{table}

\subsection{Microphysics of Dust Extinction and Interstellar Reddening}

Interstellar dust grains ($a \sim 0.01\text{--}1\,\mu\mathrm{m}$) are comparable in size to optical wavelengths, placing ISM dust extinction squarely in the \textbf{Mie scattering regime}. 

For a sightline traversing dust column density $N_d = \int n_d\,ds$, the optical depth is $\tau_\lambda = N_d \sigma_{\rm ext}(\lambda)$, giving flux attenuation $F_\lambda = F_{\lambda,0}e^{-\tau_\lambda}$. Expressed in Pogson magnitudes:
\begin{align}
	m &= M -2.5\log_{10}\left(\frac{F_m e^{-\tau_{\lambda}}}{F_{M}}\right)\implies A_\lambda = \frac{2.5}{\ln 10}\,\tau_\lambda \approx 1.086\,N_d\,\sigma_{\rm ext}(\lambda).
	\label{eq:ism_mag_extinction}
\end{align}
Because Mie scattering across the optical window satisfies $\sigma_{\rm ext}(\lambda) \propto \lambda^{-1}$, the cross-section decreases monotonically with wavelength. Blue photons suffer greater attenuation than visual or red photons ($\sigma_B > \sigma_V \implies A_B > A_V$). This differential extinction creates the \textbf{color excess} $E(B-V) \equiv A_B - A_V > 0$, causing transmitted starlight to emerge fainter and redder than its intrinsic emission. Correcting for this effect via the \textbf{Color Index Correction} $(B-V)_0 = (B-V)_{\rm obs} - E(B-V)$ and dereddening the distance modulus via $\mu_0 = (m_V - M_V)_{\rm obs} - R_V E(B-V)$ with $R_V \approx 3.1$ (as derived in Chapter~4) is therefore essential for accurate stellar characterization. A direct observational consequence is seen in dark and reflection nebulae: dense dust clouds such as Barnard 68 appear opaque in the optical but become transparent in the infrared where $\lambda \gg a$, while starlight scattered off dust around the Pleiades produces a blue reflection nebula because smaller grains scatter shorter wavelengths more effectively.
...
\begin{figure}[htbp]
	\centering
	\includegraphics{fig_ch8_line_profile_voigt.pdf}
...
\subsection{Expanding Planetary Nebulae and the Doppler-Split Line}
\label{sec:pn_expansion}

A planetary nebula is a shell of gas thrown off by a star near the end of its life. The hot core left behind, a white dwarf, ionizes the shell and makes it glow in emission lines. The shell keeps expanding outward, roughly as a sphere centered on the white dwarf, at some speed $v_{\rm exp}$, typically a few tens of $\mathrm{km/s}$.

Every point on the shell moves outward at the same speed $v_{\rm exp}$, but not every point moves the same way along our line of sight, since the shell is curved. Figure~\ref{fig:pn_expansion} shows why. The point of the shell nearest to us moves straight toward us, so its light is blueshifted. The point on the far side of the star moves straight away from us, so its light is redshifted. Points around the visible rim of the shell move sideways, across our line of sight, so their light is not shifted at all. Every other point on the shell moves at some angle in between, giving a line-of-sight speed somewhere between these two extremes.

\begin{figure}[htbp]
	\centering
	\resizebox{\linewidth}{!}{%
	\includegraphics{fig_ch8_expanding_shell_geometry.pdf}%
	}
	\caption{(a) An expanding spherical shell around a white dwarf, viewed from Earth. Arrows show the local expansion velocity, color-coded by its line-of-sight sense: blue for the near cap (moving toward Earth), red for the far cap (moving away from Earth), gray for the rim (moving across the line of sight). (b) The resulting emission-line profile, spread between $\lambda_{\min}$ (from the near cap) and $\lambda_{\max}$ (from the far cap), with the unshifted rest wavelength $\lambda_0$ marked in between.}
	\label{fig:pn_expansion}
\end{figure}

Apply the classical Doppler relation, Eq.~\eqref{eq:doppler_classical}, to each of these two extreme points in turn. The near cap moves toward Earth at $v_r=-v_{\rm exp}$, so its light lands at
\[
\lambda_{\min}=\lambda_0\left(1-\frac{v_{\rm exp}}{c}\right),
\]
and the far cap moves away from Earth at $v_r=+v_{\rm exp}$, so its light lands at
\[
\lambda_{\max}=\lambda_0\left(1+\frac{v_{\rm exp}}{c}\right).
\]
A single sharp rest-frame line, once its light from the whole shell is added together, is therefore seen as one line spread out between $\lambda_{\min}$ and $\lambda_{\max}$, as sketched in panel (b) of Figure~\ref{fig:pn_expansion}. Subtracting the two relations above cancels $\lambda_0$ from the difference and isolates the expansion speed:
\begin{keyresult}
	\begin{equation}
		\lambda_{\max}-\lambda_{\min} = \frac{2\lambda_0\,v_{\rm exp}}{c} \qquad\Longrightarrow\qquad v_{\rm exp} = \frac{c}{2}\,\frac{\lambda_{\max}-\lambda_{\min}}{\lambda_0}.
		\label{eq:pn_expansion_velocity}
	\end{equation}
\end{keyresult}
Measuring the two extreme edges of an observed emission line therefore gives the shell's expansion speed directly, with no need to know the nebula's distance.
...
\section{Problem Set: The Interstellar Medium and Gravitational Collapse}

This problem applies the Doppler-shift machinery of Section~\ref{sec:pn_expansion} to a real planetary nebula spectrum.

\subsection{Problem 1: Planetary Nebula Emission and Expansion}

\textbf{Problem Statement}: Suppose we observe the $\mathrm{H}\alpha$ line from a spherical planetary nebula, and find it has a maximum wavelength of $656.32\,\mathrm{nm}$ and a minimum wavelength of $656.24\,\mathrm{nm}$. Estimate how fast the nebula is expanding. Give your answer in both $\mathrm{km/s}$ and $\mathrm{AU/yr}$.

\textbf{Solution}: As shown in Figure~\ref{fig:pn_expansion}, the near cap of the expanding shell gives the most blueshifted wavelength, $\lambda_{\min}=656.24\,\mathrm{nm}$, and the far cap gives the most redshifted wavelength, $\lambda_{\max}=656.32\,\mathrm{nm}$. The rest wavelength $\lambda_0$ sits midway between them,
\[
\lambda_0 = \frac{\lambda_{\max}+\lambda_{\min}}{2} = \frac{656.32+656.24}{2}\,\mathrm{nm} = 656.28\,\mathrm{nm},
\]
which agrees with the known rest wavelength of $\mathrm{H}\alpha$, confirming the nebula as a whole is not moving toward or away from us on average. Using Eq.~\eqref{eq:pn_expansion_velocity},
\[
v_{\rm exp} = \frac{c}{2}\,\frac{\lambda_{\max}-\lambda_{\min}}{\lambda_0} = \frac{2.998\times10^{5}\,\mathrm{km/s}}{2}\times\frac{656.32-656.24}{656.28} \approx \boxed{18.3\,\mathrm{km/s}.}
\]
Converting to $\mathrm{AU/yr}$ using $1\,\mathrm{yr}=3.156\times10^{7}\,\mathrm{s}$ and $1\,\mathrm{AU}=1.496\times10^{8}\,\mathrm{km}$,
\[
v_{\rm exp} = 18.3\,\frac{\mathrm{km}}{\mathrm{s}}\times\frac{3.156\times10^{7}\,\mathrm{s/yr}}{1.496\times10^{8}\,\mathrm{km/AU}} \approx \boxed{3.86\,\mathrm{AU/yr}.}
\]
This is a typical expansion speed for a planetary nebula shell, consistent with observed values of order $10$--$30\,\mathrm{km/s}$.
