...
A line-emitting particle moving with line-of-sight velocity $v_\parallel$ has its rest-frame frequency $\nu_0$ Doppler-shifted according to the classical relation already used in Chapter~3 (Eq.~\eqref{eq:doppler_classical}), $\Delta\nu/\nu_0 = v_\parallel/c$. In a gas at temperature $T$, the line-of-sight velocities of individual emitters follow a one-dimensional Maxwell--Boltzmann distribution with standard deviation $\sigma_v = \sqrt{k_B T/m}$ ($m$ the particle mass), so the ensemble of Doppler-shifted photons produces a Gaussian line profile of characteristic (thermal) width
\begin{keyresult}
	\begin{equation}
	...
	\end{equation}
\end{keyresult}
Bulk, non-thermal gas motions (turbulence, infall, rotation) add an additional, generally larger, Doppler contribution with the same functional form but $\sigma_v$ replaced by the turbulent or bulk velocity dispersion; a net systemic line-of-sight velocity (e.g.\ Galactic rotation) simply shifts the line centroid rather than broadening it.
...
Model the cloud as a uniform sphere of mass $M$, radius $R$, and temperature $T$. Its gravitational self-energy, computed by assembling the sphere shell by shell ($U = -\int_0^R \frac{Gm(r)}{r}\,dm$ with $m(r) = M(r/R)^3$, $dm = 3M r^2/R^3\,dr$), is the standard result
\begin{equation}
...
The energy argument above is global and geometry-dependent (it assumed a uniform sphere); it is worth confirming the same physics from a local, dynamical perspective. Linearize the isothermal fluid equations (continuity, Euler, Poisson) about a uniform background of density $\rho_0$ at rest, writing $\rho = \rho_0 + \rho_1$, and seek plane-wave perturbations $\rho_1 \propto \exp[i(\vec k\cdot\vecr - \omega t)]$ with sound speed $c_s^2 = k_BT/(\mu m_{\rm H})$. The linearized equations combine into the dispersion relation
\begin{equation}
