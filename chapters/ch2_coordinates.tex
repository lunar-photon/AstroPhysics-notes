...
\end{equation}
For $h > 15^\circ$, the plane-parallel approximation yields:
\begin{equation}
	R \approx 58.2''\cot h_{\rm true}.
	\label{eq:refraction_cot}
\end{equation}
Near the horizon, Bennett's formula provides sub-arcsecond accuracy:
\begin{equation}
...
The finite speed of light combined with the Earth's orbital motion around the Sun at velocity $v \approx 29.8\,\mathrm{km/s}$ produces an apparent angular displacement:
\begin{keyresult}
	\begin{equation}
...
The angular separation $\theta$ between two points $(\alpha_1, \delta_1)$ and $(\alpha_2, \delta_2)$ is given by the dot product of their unit vectors:
\begin{keyresult}
	\begin{equation}
...
The rotation between the equatorial and ecliptic frames by obliquity $\epsilon$ is:
\begin{equation}
...
The transformation from equatorial coordinates $(\alpha, \delta)$ to Galactic coordinates $(l, b)$ is:
\begin{keyresult}
	\begin{align}
...
The \emph{airmass} $X$ measures the optical path length through the atmosphere relative to the zenith path:
\begin{equation}
	X \equiv \frac{\int \rho(s)\,ds}{\int \rho(z)\,dz} \approx \sec z = \frac{1}{\cos z} = \frac{1}{\sin h}.
	\label{eq:airmass_secz}
\end{equation}
For $z > 60^\circ$, Earth's curvature becomes significant, well modeled by Kasten \& Young (1989):
\begin{equation}
...
The \emph{parallactic angle} $q$ is the angle between the great circle through the celestial pole and the vertical great circle through the zenith:
\begin{equation}
