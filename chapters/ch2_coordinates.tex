...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[>=Stealth, scale=1.3, font=\small]
		...
		\coordinate (P) at (0.3, 2.6);
		\coordinate (Z) at (2.3, 1.0);
		\coordinate (S) at (-1.0, -1.8);

		\draw[dashed, gray!60] (-3,-0.4) to[out=10,in=170] (3,0.6);
		\node[gray!70, font=\scriptsize] at (2.1, 0.95) {Celestial Equator};

		\draw[dashed, astroblue!70] (-2.6,2.0) to[out=-40,in=140] (2.9,-1.6);
		\node[astroblue!80, font=\scriptsize] at (-2.05,1.9) {Horizon};

		\draw[very thick, accent] (P) to[out=-60,in=140] (Z);
		\draw[very thick, accent] (P) to[out=-100,in=100] (S);
		\draw[very thick, accent] (Z) to[out=200,in=30] (S);

		\fill (P) circle(1.5pt) node[above=2pt]{NCP};
		\fill (Z) circle(1.5pt) node[right=2pt]{Zenith ($Z$)};
		\fill (S) circle(1.5pt) node[below left=2pt]{Star ($S$)};

		\node[accent, font=\scriptsize] at ($(P)!0.5!(Z)+(0.45,0.05)$) {$90^\circ-\phi_0$};
		\node[accent, font=\scriptsize] at ($(P)!0.5!(S)+(-0.55,0.1)$) {$90^\circ-\delta$};
		\node[accent, font=\scriptsize] at ($(Z)!0.5!(S)+(0.25,-0.4)$) {$z = 90^\circ-h$};

		\node[font=\scriptsize] at ($(P)+(0.55,-0.35)$) {$H$};
		\node[font=\scriptsize] at ($(Z)+(-0.6,-0.15)$) {$A$};
		\node[font=\scriptsize] at ($(S)+(0.15,0.55)$) {$q$};
	\end{tikzpicture}
	\caption{The astronomical (Pole--Zenith--Star) triangle. Its three sides are the star's polar distance ($90^\circ-\delta$), the observer's co-latitude ($90^\circ-\phi_0$), and the star's zenith distance ($z = 90^\circ-h$); its three angles are the Hour Angle $H$ (at the pole), the Azimuth $A$ (at the zenith), and the Parallactic Angle $q$ (at the star itself, Section~\ref{sec:parallactic_angle}). The horizontal$\leftrightarrow$equatorial transformation, the rise/set condition, and the parallactic angle formula are all spherical trigonometry applied to this one triangle.}
...
Under a passive anticlockwise rotation of the $x$-$y$ axes by an angle $\theta$, standard 2D rotation gives the new coordinates $(x',y') = (x\cos\theta + y\sin\theta,\ -x\sin\theta + y\cos\theta)$, and correspondingly any angle $\alpha$ measured from the old $x$-axis becomes, measured from the new one, the angle $\alpha' \equiv \alpha - \theta$ --- i.e., writing $\theta = \alpha_{\rm Meridian}$,
\begin{equation}
	\alpha = \alpha_{\rm Meridian} + \alpha'.
	\label{eq:alpha_prime_def}
\end{equation}
Applying this rotation to the source's equatorial-frame coordinates $(\cos\delta\cos\alpha,\ \cos\delta\sin\alpha,\ \sin\delta)$ of Eq.~\eqref{eq:unit_vector_eq} (the $z$-component is untouched, since the rotation is about the polar axis), and using the angle-addition identities together with Eq.~\eqref{eq:alpha_prime_def}:
\begin{align}
	x' &= \cos\delta\cos\alpha\cos\alpha_{\rm Meridian} + \cos\delta\sin\alpha\sin\alpha_{\rm Meridian} = \cos\delta\cos(\alpha-\alpha_{\rm Meridian}) = \cos\delta\cos\alpha', \\
	y' &= -\cos\delta\cos\alpha\sin\alpha_{\rm Meridian} + \cos\delta\sin\alpha\cos\alpha_{\rm Meridian} = \cos\delta\sin(\alpha-\alpha_{\rm Meridian}) = \cos\delta\sin\alpha'.
\end{align}
So quite generally, in \emph{any} frame rotated by $\alpha_{\rm Meridian}$ from the vernal equinox,
\begin{equation}
